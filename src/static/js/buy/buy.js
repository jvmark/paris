    ...
    function ($scope, BuyService, $routeParams,ngDialog) {
      ...
      $scope.login = function(eitem) {
        BuyService.logInOrOut.login({
          ...
          $scope.isLogin = true;
          ngDialog.close();
          SUGAR.PopOut.alert('<div class="prompt"><h3>登录成功</h3></div>');
          $({}).delay(4000).queue(function() {
            SUGAR.PopOut.closeMask();
          });
        })
...
angular.module('buyOrderController', [])
  .controller('buyOrderPageCtrl', ['$scope', 'BuyService', '$routeParams',
    function ($scope, BuyService, $routeParams) {
      ...
      // 订单状态结构
      $scope.orderMaps = [
        {value: '', status: 'ALL', text: '所有'}, // 后端无此字段
        {value: -1, status: 'INVALID', text: '订单无效'},
        {value: 0, status: 'CREATED', text: '已创建'},
        {value: 4, status: 'PAID', text: '已支付'},
        {value: 8, status: 'AUDITED', text: '已审核'},
        {value: 16, status: 'HANDLED', text: '已交付'},
        {value: 17, status: 'STOCKOUT', text: '缺货异常'},
        {value: 32, status: 'DELIVERING', text: '已发货'},
        {value: 33, status: 'DELIVER_EXCEPTION', text: '配送异常'},
        {value: 35, status: 'DELIVER_FAILED', text: '未送达'},
        {value: 64, status: 'DELIVERED', text: '送达'},
        {value: 128, status: 'REFUND', text: '订单退款'},
        {value: 256, status: 'CLOSED', text: '订单关闭'},
      ];
      ...
      $scope.orderQueryMaps = [
        {value: 'bizOrderId', text: '订单号'},
        {value: 'buyerUid', text: '堆糖ID(等于买家ID)'},
        {value: 'inventoryIds', text: '商品ID'}
      ];
      ...
      $scope.getCount = function(){
        ...
        var start = 0;
        var params = {start:start,limit:limit,startDate:startDate,endDate:endDate,orderStatus:orderStatus,timeType:timeType};
      ...
      function queryOrder(start) {
        ...
        BuyService.order.export(config).then(function(data) {
          ...
          var baseurl = '#/order/';
          var searcharg = {limit:limit,startDate:startDate,endDate:endDate,orderStatus:orderStatus};
  ...
  .controller('buyShelvesEditController', ['$scope', 'BuyService', '$routeParams','$location',
    function ($scope, BuyService, $routeParams,$location) {
      function init(){
        ...
        BuyService.inventory.get({params:_params}).then(function(data_){
          ...
          angular.forEach($scope.inventory.pictures, function(k, v) {
            ...
            $scope.inventory.picturesObj.push({'img':$scope.inventory.pictures[v]});
          });
          ...
          angular.forEach($scope.inventory.detail_desc_pics, function(k, v) {
            ...
            $scope.inventory.detailPicturesObj.push({'img':$scope.inventory.detail_desc_pics[v]});
          });
        });
      }
      init();
      ...
      $scope.upload = function(fileInex_,fileId_,resultObj_) {
        ...
        xhr.open("POST", "http://www.duitang.com/napi/upload/photo/", true);
        xhr.withCredentials = true;
        // 发送表单数据
        xhr.send(formData);
      }

      $scope.addPic = function(picList_){
        picList_.push({'img':''});
      }
      $scope.removePic = function(picList_){
        picList_.splice(this.$index, 1);
      }
      ...
      $scope.saveShelves = function(){
        ...
        var _params = {
          ...
          "inventory_status": $scope.inventory.inventory_status,
          "count": $scope.inventory.modifyCount,
  ...
  .controller('buyPromotionCtrl', ['$scope', 'BuyService', '$location',
    function($scope, BuyService, $location) {
      ...
      $scope.create = function(){
        window.open('/buy/#/promotion/update/');
      }
...
.controller('SPUAddCtrl', ['$scope','BuyService','$location','ngDialog', function ($scope, BuyService,$location,ngDialog) {
		function init(){
			...
			$scope.relatedBlogs = '';
			$scope.mainPic = {};
			$scope.banners = [{'desc':'','link':''}];
			$scope.detailPics = [{'img':''}];
      $scope.barCodes = [{'barCode':'', 'count':''}];
      $scope.pictureList = [{'img':''}];
      $scope.packPicsList = [{'img':''}];
      $scope.desc = '';
      $scope.caption = '';
      $scope.marketPrice = '';
...
.controller('SPUEditCtrl', ['$scope','BuyService', '$routeParams','$location','ngDialog', function ($scope, BuyService,$routeParams,$location,ngDialog) {
    ...
    function getSpuInfo(){
        ...
        if(!$scope.spuInfo.barCodes){
          $scope.spuInfo.barCodes = [{"barCode":"","count":""}];
        ...
        //将图片放到数组对象中$scope.spuInfo.descPics{img}
        	...
        	$scope.spuInfo.descPics.push({'img':$scope.spuInfo.detailDesc[v]});
	    	});
        //将图片放到数组对象中$scope.spuInfo.pictures{img}
        angular.forEach($scope.spuInfo.pictures, function(k, v) {
          $scope.spuInfo.pictureList.push({'img':$scope.spuInfo.pictures[v]});
        });
        //将图片放到数组对象中$scope.spuInfo.packPics{img}
        angular.forEach($scope.spuInfo.packPics, function(k, v) {
          $scope.spuInfo.packPicsList.push({'img':$scope.spuInfo.packPics[v]});
        });
        //根据spu类型的id，获取属性列表
        BuyService.spu.getPropertiesList({
